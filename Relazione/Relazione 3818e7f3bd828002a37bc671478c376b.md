...
Aziende private: aziende operanti nel settore di vendita/leasing di veicoli elttrici per individuare le aree e le tecnologie che necessitano di maggiore intervento 
...
I dati sono stati repertiti tramite download di un file .CSV da https://www.piattaformaunicanazionale.it/, dove i dati hanno il seguente formato:
...
WHERE {
  ...
  SERVICE <x-sparql-anything:location=data.csv,csv.headers=true> {
	...
	# Crezione degli URI deigli individui
  # Città
  BIND(
      IRI(CONCAT(
